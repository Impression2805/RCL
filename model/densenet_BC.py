...
class DenseNet3(nn.Module):
    def __init__(self, depth=100, num_classes=10, growth_rate=12,
        ...
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.relu = nn.ReLU(inplace=True)
        # self.cosfc = CosClassifier(num_features=in_planes, num_classes=num_classes)
        # self.sfc = StochasticClassifier(num_features=in_planes, num_classes=num_classes, temp=16)
    ...
    def forward(self, x, feature_output=False, stochastic=False, test=False, cos=False):
        ...
        # if stochastic:
        #     y = self.sfc(feature, test=test)
        # elif cos:
        #     y = self.cosfc(feature)
        # else:

        # score = F.linear(F.normalize(feature, p=2, dim=1), F.normalize(self.fc.weight.data, p=2, dim=1))
        # y = score * 16
    ...
    def forward_threshold(self, x, threshold=1e10):
        ...
        feature = out.view(-1, self.in_planes)
        feature_original = out_original.view(-1, self.in_planes)
        return self.fc(feature), self.fc(feature_original)
    ...
    def forward_manifold(self, x, length_size=0):
        ...
        x = F.avg_pool2d(x, 8)
        feature = x.view(-1, self.in_planes)
        x = self.fc(feature)
        return x, lam_return
