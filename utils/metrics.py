...
def calc_metrics(args, loader, label, label_onehot, model, criterion, old_network=None, ood_loader=None):
    acc, softmax, correct, logit, conf_correct, conf_wrong, ece_correct, ece_wrong, nll_correct, nll_wrong = get_metric_values(args, loader, model, criterion, old_network)
    # acc, softmax, correct, logit, conf_correct, conf_wrong, ece_correct, ece_wrong, nll_correct, nll_wrong, conf_ood = get_metric_values(
        # args, loader, model, criterion, ood_loader)
    # aurc, eaurc
    ...
    #return acc, auroc * 100, aurc * 1000, conf_correct* 100, conf_wrong* 100, conf_ood* 100
...
# Get softmax, logit
def get_metric_values(args, loader, model, criterion, old_network, ood_loader=None):
    model.eval()
    with torch.no_grad():
        total_loss = 0.
        total_acc = 0.
        ...
        for input, target, idx in loader:
            ...
            if old_network is not None:
                _, feature_new = model(input, feature_output=True)
                _, feature_old = old_network(input, feature_output=True)
                feature = (feature_old + feature_new) / 2
            ...
            # # output = model.forward_pca(input) #### for openmix cos
            # if args.method == 'classAug' or args.method == 'OpenMix' or args.method == 'RejOE' or args.method == 'OpenMix-CRL' or args.method == 'OpenMix-flat' or args.method == 'OpenMix_manifold':
        ...
        # conf_ood = []
        # for batch_idx, (input, target) in enumerate(ood_loader):
        #     input = input.cuda()
        #     target = target.long().cuda()
        #     output = model(input)
        #     prob, _pred = F.softmax(output, dim=1).max(1)
        #     p = prob.detach().cpu().view(-1).numpy()
        #     # print(888888888888888888)
        #     # print(p)
        #     conf_ood.append(p)
        # conf_ood = np.concatenate(conf_ood, axis=0)
        # conf_ood = np.mean(conf_ood)

    # return total_acc, list_softmax, list_correct, list_logit, conf_correct, conf_wrong, ece_correct, ece_wrong, nll_correct, nll_wrong, conf_ood
...
def get_metric_values_DUQ(args, loader, model):
    model.eval()
    with torch.no_grad():
        total_acc = 0.
        accuracy = 0.
        ...
        for input, target, idx in loader:
            input = input.cuda()
            target = target.long().cuda()
            output = model.forward_DUQ(input)
            kernel_distance, pred = output.max(1)
            labels_list.append(target.cpu())
            prob = -kernel_distance
            total_acc += pred.eq(target.data.view_as(pred)).sum()
            list_softmax.extend(prob.detach().cpu().view(-1).numpy())
            ...
            for j in range(len(pred)):
                ...
                list_correct.append(cor)
        total_acc = 100. * total_acc.item() / len(loader.dataset)
    return total_acc, list_softmax, list_correct
